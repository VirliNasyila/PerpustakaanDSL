...
public class check_return_date_format_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  public check_return_date_format_NonTypesystemRule() {
  }
  public void applyRule(final SNode item, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if (SPropertyOperations.getString(item, PROPS.key$EFxw) == "tanggal") {
      String t = SPropertyOperations.getString(SLinkOperations.getTarget(item, LINKS.value$F0aS), PROPS.text$gdDF);
      if (!(REGEXP.matcher(t).matches())) {
        {
          final MessageTarget errorTarget = new NodeMessageTarget();
          IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(SLinkOperations.getTarget(item, LINKS.value$F0aS), "Format tanggal harus YYYY-MM-DD", "r:91ccb5e2-c7b3-4094-bfb0-e8378f032fa4(perpustakaan.language.typesystem)", "6062119405061192828", null, errorTarget);
        }
      }
    }
  }
  ...
  private static final Pattern REGEXP = Pattern.compile("\\\\d\\{4\\}-\\\\d\\{2\\}-\\\\d\\{2\\}", 0);
